...
This document lists all environment variables required for the Agentic PM
Workbench.
...
| Variable              | Required  | Description                        | Example                 |
| --------------------- | --------- | ---------------------------------- | ----------------------- |
| `DYNAMODB_ENDPOINT`   | No        | Local DynamoDB endpoint (dev only) | `http://127.0.0.1:4566` |
| `LOCALSTACK_ENDPOINT` | No        | LocalStack endpoint (dev only)     | `http://127.0.0.1:4566` |
| `NEXTAUTH_URL`        | Yes (Web) | NextAuth.js base URL               | `http://localhost:3000` |
| `DIGEST_EMAIL`        | No        | Daily digest recipient email       | `admin@example.com`     |
...
TABLE_NAME=AgenticPM
TABLE_ARN=arn:aws:dynamodb:ap-southeast-2:000000000000:table/AgenticPM
